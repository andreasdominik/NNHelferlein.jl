...
The one-argument version can be usewd, if encoder dimensions and decoder
...
### Arguments:
+ `h_t`:    decoder hidden state. If ndims(h_t)==1, length of h_t
            equals the number of decoder units. If ndims(h_t)==2,
            h_t includes the states for a minibatch of samples and has
            tha size [n_units, n_mb].
+ `h_enc`:    ecoder hidden states, 2d or 3d. If 2d,  `h_enc` is a
